...
/**
 * Browser hostname for license checks (matches window.location.hostname; no port).
 */
if (!function_exists('rise_request_hostname')) {

    function rise_request_hostname(): string {
...
if (!function_exists('rise_skip_fairsketch_license')) {
    ...
    function rise_skip_fairsketch_license(): bool {
...
/**
 * Same output as appHash() in assets/js (used for app_verification_key validation).
 */
if (!function_exists('rise_app_hash')) {

    function rise_app_hash(string $host, bool $extended = true): string {
        $m = md5($host);
        if (!$extended) {
            return $m;
        }
        $m = substr($m, 5, strlen($m) - 12);
        $c = substr($m, 0, 6);
        $m = substr($m, 6) . $c;

        return strtoupper('kxgyqylvv' . $m . 'fbuyatgp');
    }
}

/**
 * Single-segment key shape expected by _verfy: 5-char pad + rise_app_hash + 5-char pad.
 */
if (!function_exists('rise_build_local_app_verification_key')) {

    function rise_build_local_app_verification_key(?string $host = null): string {
        if ($host === null || $host === '') {
            $host = rise_request_hostname();
        }
        $hash = rise_app_hash($host, true);
        $pad = 'l09l9';
