...
// Paths must be absolute from this file — relative "../" breaks when CWD is not install/ (e.g. php built-in server).
$projectRoot = dirname(__DIR__);
$installLogPath = $projectRoot . '/writable/logs/install.log';
...
try {
    ...
    if (isset($_POST)) {
        ...
        $db_file_path = $projectRoot . '/app/Config/Database.php';
        $config_file_path = $projectRoot . '/app/Config/App.php';
        $db_sql_file_path = __DIR__ . '/database.sql';
        $index_file_path = $projectRoot . '/index.php';
        ...
        // PHP 8.1+ mysqli can throw mysqli_sql_exception; "localhost" uses a Unix socket — wrong socket path => "No such file or directory".
        mysqli_report(MYSQLI_REPORT_OFF);
        $mysqli = @new mysqli($host, $dbuser, $dbpassword, $dbname);

        if ($mysqli->connect_errno) {
            $hint = (stripos($host, 'localhost') !== false)
                ? ' Try host 127.0.0.1 instead of localhost if MySQL is only listening on TCP.'
                : '';
            echo json_encode(array(
                "success" => false,
                "message" => $mysqli->connect_error . $hint,
            ));
            exit();
        }
    ...
    error_log(date('[Y-m-d H:i:s e] ') . $detail . PHP_EOL, 3, $installLogPath);
    $safe = $ex->getMessage();
    if (stripos($safe, 'No such file or directory') !== false) {
        $safe .= ' (Database host: if you used "localhost", switch to "127.0.0.1" — localhost uses a Unix socket on many systems.)';
    }
    echo json_encode(array(
        "success" => false,
        "message" => $safe . " Full details: /writable/logs/install.log",
    ));
}
...
function verify_rise_purchase_code($code) {
    ...
    $config_file_path = dirname(__DIR__) . '/app/Config/Rise.php';
    if (is_file($config_file_path)) {
        $config_file = file_get_contents($config_file_path);
